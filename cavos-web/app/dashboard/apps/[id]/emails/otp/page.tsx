...
import { ArrowLeft, AlertCircle, CheckCircle2, Copy, ImageIcon, Loader2, Mail, RotateCcw, ShieldCheck, Upload } from 'lucide-react'
...
const PLACEHOLDERS = [
  { key: '{{otp_code}}', label: 'otp_code', description: 'Six-digit sign-in code' },
  { key: '{{expires_minutes}}', label: 'expires_minutes', description: 'Code expiry window' },
...
export default function OtpEmailPage() {
    ...
    const fetchApp = async () => {
      try {
        ...
        setFormData({
          ...
          email_otp_template_html: data.app.email_otp_template_html || DEFAULT_TEMPLATE,
        })
      } catch {
  ...
  const handleSave = async () => {
    ...
    try {
      const templateToSave = formData.email_otp_template_html === DEFAULT_TEMPLATE ? '' : formData.email_otp_template_html
      const res = await fetch(`/api/apps/${appId}`, {
        method: 'PATCH',
        ...
        body: JSON.stringify({
          ...
          email_otp_template_html: templateToSave,
        }),
      })
  ...
  const insertPlaceholder = useCallback((placeholder: string) => {
    ...
    const value = formData.email_otp_template_html
    const next = value.substring(0, start) + placeholder + value.substring(end)
    setFormData(prev => ({ ...prev, email_otp_template_html: next }))
    requestAnimationFrame(() => {
      el.focus()
      el.setSelectionRange(start + placeholder.length, start + placeholder.length)
    })
  }, [formData.email_otp_template_html])
  ...
  const previewHtml = useMemo(() => {
    return (formData.email_otp_template_html || DEFAULT_TEMPLATE)
      .replace(/\{\{otp_code\}\}/g, '123456')
      .replace(/\{\{expires_minutes\}\}/g, '10')
  ...
  }, [formData.email_otp_template_html, formData.name, formData.logo_url])
  ...
  const lineCount = formData.email_otp_template_html.split('\n').length
  ...
  return (
    <div className="flex flex-col gap-5 pb-4">
      ...
      <div>
        <h1 className="text-xl font-semibold tracking-tight text-[#0A0908] mb-1">OTP Email</h1>
      ...
      <div className="flex items-center justify-between pt-1">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={handleSave} disabled={saving || uploading} className="inline-flex items-center gap-2 h-9 px-4 rounded-xl text-sm font-semibold bg-[#0A0908] text-white hover:bg-black/80 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed transition-all">
            {saving ? <><Loader2 className="w-3.5 h-3.5 animate-spin" /> Saving...</> : <><Mail className="w-3.5 h-3.5" /> Save Settings</>}
          </button>
        </div>
      </div>
    </div>
