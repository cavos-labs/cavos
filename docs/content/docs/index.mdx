...
chains. **Starknet is available today**; Solana and Stellar follow on the same
...
| **Chain adapter** | The per-chain layer (address derivation, signature encoding, deploy calls). `StarknetAdapter` today; Solana / Stellar slot in behind the same interface. See [Chains](/docs/chains). |
