...
chains. **Starknet and Solana are available today**; Stellar follows on the same
interface. See [Chains](/docs/chains).
...
| **Chain adapter** | The per-chain layer (address derivation, signature encoding, deploy calls). `StarknetAdapter` and `SolanaAdapter` today; Stellar slots in behind the same interface. See [Chains](/docs/chains). |
