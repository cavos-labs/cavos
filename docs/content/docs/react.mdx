  ...
  return (
    <CavosProvider
      config={{
        ...
        network: "testnet",
        appSalt: "my-app",
        socialRecovery: true,       // enclave DEK wrap (multi-chain OK)
...
| Prop | Description |
|------|-------------|
| `config` | Chains, network, `appSalt`, `deviceApproval` (`"enclave"` \| `"passkey"` — passkey is one chain), `socialRecovery`, `vault` (on by default with `appId`; see [Cavos vault](/docs/vault)). |
...
| `authError: string \| null` | Last unrecoverable auth/connect error. Null while healthy. `CavosAuthModal` shows it on the sign-in screen; from 0.2.3 that includes a connect that fails while the "Connecting with…" spinner is up. |
...
| Method | Description |
|--------|-------------|
| `addSigner(pubkey)` | Authorize another device signer (gasless `add_signer`). **Starknet.** Throws on native Solana. On grandfathered Stellar use `wallet.removeDevice` / Horizon APIs instead. |
| `removeSigner(pubkey)` | Revoke a device signer (gasless `remove_signer`) — the "this wasn't me" escape hatch. Must run from a device that is already authorized, and cannot revoke itself. Throws on native Solana. On **Stellar** use `wallet.removeDevice({ slotId })` instead. |
| `listDevices()` | Device signer pubkeys currently authorized. **Starknet.** Throws on native Solana. On Stellar call `wallet.listDevices()` for Horizon signer keys (grandfathered extra signers). |
| `enrollPasskey(passkey, params)` | Low-level passkey enrollment when you drive WebAuthn yourself. Most apps want `enrollPasskeyDefault()`. Throws on native Solana (a passkey is not an on-chain approver there). |
| `enrollPasskeyDefault()` | Add a passkey **after** sign-up. Native Solana/Stellar: stores a PRF-encrypted copy of the MasterDEK. Starknet: on-chain `addApprover`. Sign-up never prompts. |
| `approveDeviceWithPasskey()` | New device with no wrap, when the user dismissed the prompt: opens the passkey's DEK copy (native Solana/Stellar) or approves the Starknet / grandfathered-Stellar path. |
| `setupRecovery()` | Starknet (and grandfathered Stellar). Native Solana/Stellar is a no-op. **Resolves with the code** when it applies. |
| `submitSocialRecoveryToken(idToken)` | Drive enclave enroll/unwrap with the provider's raw `id_token`. Solana, Stellar, and Starknet. See [Hardware-isolated recovery](/docs/hardware-isolated-recovery#with-your-own-auth). |
| `recover(code)` | Recover access after losing every device. **Starknet.** Native Solana/Stellar throw — restore with enclave login or `approveDeviceWithPasskey()`. |
| `resendDeviceApproval()` | Re-request the device-approval email for the current pending request. **Starknet.** |

When hardware-isolated social recovery is enabled, `<CavosProvider>` enrolls
after a fresh login (reseals the MasterDEK if this device already has it) and
unwraps on a device that does not. Passkey is opt-in via
`enrollPasskeyDefault()` — `secureStep` defaults to `'off'`.
...
| `logout()` | Clears the persisted identity and session token, and tells the [vault](/docs/vault) to let go of the keys it unlocked. Stored keys stay, so the next login is silent. Does **not** sign the user out of Google or Apple. |
...
| `emailMode` | `"magic-link" \| "otp"?` | How the email provider authenticates. Defaults to `magic-link`. On the enclave (`deviceApproval: "enclave"`, the default with `socialRecovery`) the modal always uses `magic-link` from 0.2.3: an email code yields no token the enclave accepts. See [Authentication](/docs/auth#email-otp). |
...
| `secureStep` | `"optional" \| "required" \| "off"?` | Passkey screen after a new account. **`'off'` is the default** — login never prompts. |
