...
| `submitSocialRecoveryToken(idToken)` | Drive social recovery with a provider `id_token` your own login obtained. Enrols when the device is ready, recovers when it is not. See [Recovery](/docs/recovery#with-your-own-auth). |
