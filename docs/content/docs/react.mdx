...
| Prop | Description |
|------|-------------|
| `config` | Chains, network, `appSalt`, `deviceApproval` (`"enclave"` \| `"passkey"` — passkey is one chain), `socialRecovery`. |
...
| `logout()` | Clears the persisted identity and session token only. IndexedDB MasterDEK wraps and device keys stay. Does **not** sign the user out of Google or Apple. |
