...
| `enrollPasskey(passkey, params)` | Low-level passkey enrollment when you drive WebAuthn yourself. Most apps want `enrollPasskeyDefault()`. Throws on native Solana (PRF is not an on-chain approver). |
| `enrollPasskeyDefault()` | Create the passkey **after** login. Native Solana/Stellar: PRF is the MasterDEK KDF. Starknet: on-chain `addApprover`. Login never prompts. |
| `approveDeviceWithPasskey()` | New device with no wrap: reconnect with `{ passkey: true }` (native Solana/Stellar) or approve the Starknet / grandfathered-Stellar path. |
