...
<Callout type="info">
On **Solana and Stellar**, call the wallet directly instead of the context's
`execute`: narrow on `wallet.chain`, then `wallet.execute(amount, destination)`.
See [After sign-in](/docs/post-login) and [Chains](/docs/chains).
</Callout>
