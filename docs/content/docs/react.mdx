...
| `enrollPasskeyDefault()` | Enroll a synced passkey. On Starknet/Solana, registers an on-chain approver for new devices. On Stellar, adds a weight-1 Horizon signer (not 2FA — can spend). Works on `undeployed` (pending until first execute) and `ready` wallets. |
...
| `submitSocialRecoveryToken(idToken)` | Drive social recovery with a provider `id_token` your own login obtained. Enrols when a **Starknet or Solana** device is ready, recovers when it is not. No-op for classic Stellar. See [Hardware-isolated recovery](/docs/hardware-isolated-recovery#with-your-own-auth). |
...
automatically enrolls a ready **Starknet or Solana** wallet after fresh
authentication and restores an unregistered device on those chains through the
attested enclave. Classic Stellar is skipped: authorize a new `G…` device with
`approveDeviceWithPasskey()` or a recovery code.
