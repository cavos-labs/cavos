...
<Callout type="info">
**Stellar uses the same idea, a different API.** Devices are Horizon signer
keys, so `listDevices()` / `removeSigner()` on the hook throw there. Call
`wallet.listDevices()` and `wallet.removeDevice({ slotId })` after narrowing on
`wallet.chain === "stellar"`. Revocation drops that one signer. See
[Stellar → Manage devices](/docs/stellar#manage-devices).
</Callout>
