...
that already controls the wallet. Chain-native account state—not the Cavos
...
## Host a Starknet approval page

<Callout type="warning">
**For this Starknet flow, Cavos does not host an approval page.** Each integrating app must build its own
    ...
    fetch(`${BACKEND_URL}/api/devices/request?id=${requestId}`)
      ...
      .then((data) => {
        ...
        setConfig({
          ...
          appId: data.app_id,
          // Client-visible: use only an app/environment-scoped sponsorship key.
