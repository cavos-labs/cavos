...
Optional, opt-in per environment, on **Starknet and Solana**. Classic Stellar
does not use the enclave. See [Hardware-isolated recovery](/docs/hardware-isolated-recovery).
...
| `PasskeyPrf`, `PasskeyPrfOptions`, `PasskeyPrfEnrollParams` | WebAuthn PRF helper (derives the Stellar extra signer). |
...
| `NativeDeviceSigner` | Secure Enclave/Keystore P-256 signer (Starknet/Solana). |
