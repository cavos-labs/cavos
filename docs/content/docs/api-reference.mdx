...
| Export | Description |
|--------|-------------|
| `SocialRecoveryClient` | Client for the Confidential Space recovery service. `prewarm()` reserves a warm enclave worker, `enroll({ walletAddress, credential, stellarDek? })`, `recover({ walletAddress, credential, authorizations?, stellarRecipientPublicKey? })`, `confirmEnrollment(sessionId, txHash)`. |
| `SocialRecoveryClientOptions` | `{ appId, attestation, environment?, backendUrl?, prewarm? }`. Pass a reservation from a previous `prewarm()` to reuse it. |
| `SocialRecoveryPrewarm` | `{ prewarmId, claimToken, expiresAt }` — a single-use reservation for an empty, already-attested worker. Carries no identity or wallet data. |
| `AttestationPolicy` | Which enclave you accept: `audience`, `imageDigest` (a digest or a list, to overlap a rollout), `projectNumber`, `serviceAccount`. Never learned from the API being attested. |
...
| `DEFAULT_SOCIAL_RECOVERY_STARKNET_CLASS_HASH` | DeviceAccount class declaring the social-recovery entrypoints, used to upgrade pre-feature Starknet accounts. |
