...
interface ConnectOptions {
  ...
  stellarSourceKeypair?: StellarKeypair;// relayer-funded source (advanced)
  factoryId?: string;                   // account factory contract override
}
...
- `chain === "stellar"` → `CavosStellar` instance (testnet; mainnet deploys next).
...
Solana uses `CavosSolana.recover` and Stellar uses `CavosStellar.recover`, same model.)
...
| `approveDeviceEverywhere(wallets, passkey)` | Approve this device across multiple chains with a **single** passkey prompt. Only touches wallets whose `status === "needs-device-approval"`. Returns `{ chain, transactionHash }[]`. |
| `PasskeyApprovable` | Structural interface implemented by `Cavos`, `CavosSolana`, `CavosStellar` — what `approveDeviceEverywhere` operates on. |
...
Testnet today; mainnet deploys next.

| Export | Description |
|--------|-------------|
| `CavosStellar` | High-level Stellar client — the analogue of `Cavos`. `connect`, `execute(amount, destination)` (native XLM transfer, stroops), `executeTransfer(tokenId, amount, destination)` (any SEP-41 token), `balance(tokenId?)`, `addSigner`, `enrollPasskey`, `addApprover`, `approveThisDeviceWithPasskey`, `setupRecovery`, static `recover`. Returned by `Cavos.connect({ chain: "stellar" })`. |
| `StellarAdapter`, `StellarAdapterOptions` | Low-level adapter: account derivation via the factory, secp256r1 signature encoding for Soroban `__check_auth`. |
| `StellarRelayer` | Cavos gasless sponsoring relayer for Soroban (signs/submits via a relayer source). Activated by `appId`. |
| `ConnectStellarOptions`, `RecoverStellarOptions` | Option types for `CavosStellar.connect` / `.recover`. |
| `STELLAR_NETWORKS`, `StellarNetwork` | Network configs (`stellar-testnet`). |
| `sec1Pubkey` | Encode a device public key as a Stellar SEC1 key. |
| `encodeLowSSignature` (alias `encodeStellarLowSSignature`) | Low-s normalized signature encoding for Soroban. |
| `deviceSignatureScVal` | Wrap a device signature as a Soroban `ScVal`. |
