...
| `SocialRecoveryClientOptions` | `{ appId, attestation, environment?, backendUrl? }`. |
| `AttestationPolicy` | Which enclave you accept: `{ pcr0 }`, the SHA-384 measurement of the enclave image (a value or a list, to overlap a rollout). Never learned from the API being attested. |
...
| `isAcceptedImageDigest(attested, expected)` | Whether an attested image digest matches the pinned policy. Exact match; fails closed on a missing digest or an empty list. |
...
| `NativeCavosAuth` | Google, Apple, OTP, and magic-link auth through mobile callbacks. |
