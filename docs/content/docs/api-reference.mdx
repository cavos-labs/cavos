...
Classic `G…` multisig account (self-custodial, no backend/registry). This is the
sole Stellar implementation — the earlier Soroban `C…` device-account path was
removed. Live on testnet and mainnet.
