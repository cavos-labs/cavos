...
interface ConnectOptions {
  ...
  environment?: "development" | "production"; // Cavos console environment; defaults to production
  ...
  legacyDeviceApproval?: boolean;       // opt back into the pre-passkey approval flow
  ...
  createStellarDeviceKey?: (keyId: string) => Promise<DeviceUnwrapKey>; // native / tests
}
...
| `cavos.signMessage(message): Promise<MessageSignature>` | Sign an arbitrary message off-chain with the device key (secp256r1). Uniform return across chains. See [Wallet actions](/docs/wallet-actions). |
...
| `cavos.removeSigner(pubkey): Promise<{ transactionHash }>` | Revoke a device key on-chain. Must be called from an authorized device, and cannot revoke itself. |
| `cavos.upgrade(classHash): Promise<{ transactionHash }>` | Migrate the account to a newer declared `DeviceAccount` class. |
...
| `cavos.setupRecovery(code): Promise<{ transactionHash } \| undefined>` | Register a passphrase-derived backup signer. Idempotent. `CavosSolana` and `CavosStellar` expose the same method with their own factor — see [Recovery codes](/docs/recovery-codes). |
...
| Export | Description |
|--------|-------------|
| `CavosAuth` | Hosted login: `getGoogleOAuthUrl`, `getAppleOAuthUrl`, `handleCallback`, `sendOtp`, `verifyOtp`, `sendMagicLink`, `authenticate`. Also `useExternalSocialRecoveryToken(idToken)` to drive social recovery from your own login — rejects anything not issued by a provider the enclave accepts. |
...
Optional, opt-in per environment. See [Hardware-isolated recovery](/docs/hardware-isolated-recovery) for the trust
model — this is hardware-isolated, non-custodial recovery, not trustless recovery.

| Export | Description |
|--------|-------------|
| `SocialRecoveryClient` | Client for the recovery enclave. `enroll({ walletAddress, credential, stellarDek? })`, `recover({ walletAddress, credential, authorizations?, stellarRecipientPublicKey? })`, `confirmEnrollment(sessionId, txHash)`. Two synchronous round trips; there is nothing to warm up. |
| `SocialRecoveryClientOptions` | `{ baseUrl, appId, attestation, environment? }`. |
| `AttestationPolicy` | Which enclave you accept: `{ pcr0 }`, the SHA-384 measurement of the enclave image — a single value or a list, so a rollout can publish the new measurement alongside the deployed one. Never learned from the API being attested. |
| `DEFAULT_SOCIAL_RECOVERY_ATTESTATION` | The policy for the enclave Cavos operates, pinned in this package. What `socialRecovery: true` uses. |
| `enrollHardwareIsolatedRecovery`, `recoverHardwareIsolatedDevice` | Chain-aware coordinators wrapping enroll/recover, including timelock finalization. |
| `CoordinatedRecoveryResult` | Result of a coordinated recovery, including a `readyAt` when a timelock applies. |
| `SocialRecoveryCredential`, `createSocialRecoveryCredential` | The in-memory social credential handed to the enclave. Never persisted. |
| `SocialRecoveryProvider`, `SocialRecoveryAction`, `SocialRecoveryResult` | `"google" \| "apple" \| "email"`, `"enroll" \| "recover"`, and the session outcome. |
...
| `serializeInstructions`, `compressedPubkey`, `encodeLowSSignature`, `buildSecp256r1Instruction`, `anchorDiscriminator` | Low-level Solana helpers (`serializeInstructions` produces the canonical Borsh bytes an `executeInstructions` signature commits to). |
...
| Export | Description |
|--------|-------------|
| `CavosStellar` | High-level Stellar client — the analogue of `Cavos`. `connect`, `execute(amount, destination)` (native XLM transfer, stroops), `signMessage(message)` (off-chain, **ed25519** control key), `signTransaction(amount, destination)` (signed inner XDR, not submitted), `balance()` (XLM, stroops), `enrollPasskey(prfOutput)` (add a WebAuthn-PRF passkey unlock factor), `setupRecovery(code)` (add a recovery-code unlock factor), `approveThisDeviceWithPasskey(prfOutput)`, `approveThisDeviceWithRecovery(code)`, `hasPasskey`, `isReady`, `controlAddress`. Soroban + assets: `invokeContract({ contractId, method, args })`, `signXdr(unsignedXdr)` (wallet-adapter seam — sign an externally-built XDR without submitting), `addTrustline(asset)`, `tokenBalance(asset)`. Device management: `listDevices()` (envelope slot ids), `removeDevice({ slotId, passkeyPrfOutput?, recoveryCode? })` (rotates the control key — see [Stellar](/docs/stellar#manage-devices)). Returned by `Cavos.connect({ chain: "stellar" })`. |
...
| `STELLAR_NETWORKS`, `StellarNetwork`, `HORIZON_URL`, `SOROBAN_RPC_URL`, `XLM_DECIMALS` | Network configs, Horizon endpoints, and the Soroban RPC endpoints used for contract invocation (`stellar-testnet`, `stellar-mainnet`). |
| `TrustlessWorkEscrow`, `deployEscrow`, `buildEscrowScVal` | Typed wrapper for [Trustless Work](https://trustlesswork.com) escrows, driven by a Cavos `G…` account as a `require_auth` role signer. Types: `EscrowInput`, `MilestoneInput`, `EscrowState`. See [Stellar](/docs/stellar#trustless-work-escrows). |
...
| `MessageSignature` | `{ signature: Uint8Array; publicKey: string; curve: "secp256r1" \| "ed25519" }` — off-chain message signature. See [Wallet actions](/docs/wallet-actions). |
...
| `Cavos` | The native connect entry point (`Cavos.connect(options)` with `NativeConnectOptions`), wired to the platform signer and passkey adapters. |
| `NativeCavosAuth`, `NativeCavosAuthError` | Google, Apple, OTP, and magic-link auth through mobile callbacks, plus the typed error (`NativeCavosAuthErrorCode`) they reject with. |
| `NativeDeviceSigner` | Secure Enclave/Keychain or Android Keystore P-256 signer. |
| `NativeDeviceUnwrapKey` | Separate P-256 ECDH key for Stellar envelopes. |
| `NativePasskeySigner`, `NativePasskeyPrf` | AuthenticationServices/Credential Manager passkey adapters. |
| `getNativeCapabilities()` | Reports signing, ECDH, passkey, and PRF support (`NativeCapabilities`, `NativeSecurityLevel`). |
| `getCavosNativeModule()` | The underlying Expo native module, for capability checks you drive yourself. |
| `deleteDeviceKeys(keyId)` | Explicitly removes native device keys; `logout()` preserves them. |
| `approveDeviceEverywhere` | Shared Starknet/Solana passkey approval helper. |

See [React Native](/docs/react-native) for installation and native configuration.
...
| Export | Description |
|--------|-------------|
| `CavosProvider` | Wrap your app once; manages one chain's wallet + mounts the auth modal. Props: `config: CavosConfig`, `modal?: CavosModalConfig`, `identity?: Identity \| null` (bring your own auth — disables Cavos login), `children`. |
| `validateCavosConfig(config)` | Config problems as `CavosConfigProblem[]` (`code`, `level`, `message`). Run in the provider automatically in development; exported for your own setup screen or CI. |
| `checkAppSaltDrift(config, storage?)` | Reports an `appSalt` that changed since this browser last connected — addresses derive from it, so a change silently moves users to new wallets. Silent on a first run. |
| `formatConfigProblems(problems)` | Render problems as console-ready lines. |
| `useCavos()` | Primary hook — full wallet surface: state (`address`, `wallet`, `chain`, `walletStatus`, `user`, `isAuthenticated`), login, `execute` (Starknet), `signMessage`, device management (`addSigner`, `removeSigner`, `listDevices`), recovery wrappers, `logout`. |
...
| `ApproveDevicePage`, `RevokeDevicePage` | The two device-lifecycle pages, ready to mount. Props: `configs: CavosConfig[]` (every chain your app supports — the request's own chain is selected), `backendUrl?`, `paramName?`, `className?`, and `children?` as a render prop to replace the default markup. |
| `DeviceFlowState` | What the render prop receives: `{ status, request, error, isSelf, signIn, submit }`. `status` is `loading \| error \| needs-signin \| ready \| submitting \| done`. |
| `chainForNetwork(network)` | Maps a request's concrete network (`sepolia`, `solana-devnet`, `stellar-mainnet`, …) back to its `Chain`. |
| `configForNetwork(network, configs)` | Picks the config for a wallet's chain. Used internally by the pages; exported for hosts that mount the provider themselves. Throws rather than returning a config for the wrong chain. |
...
| `base64urlEncode`, `webauthnDigest`, `recoverCandidatePublicKeys`, `batchChallenge`, `lowS` | WebAuthn helpers: base64url encoding, the digest a passkey assertion signs, public-key recovery from an assertion, the batched challenge used to approve several chains with one prompt, and low-S normalization. |
