...
Paying Stellar reserves and fees **in a token** is a different package:
[`@cavos/reserve`](/docs/reserve). It is not exported from `@cavos/kit`.
...
Connect **never deploys** Starknet or Stellar accounts. It derives addresses and
checks on-chain status; those accounts are created on the first `execute`.
Native **Solana** is a system account: `ready` once this device unwraps the
spend key.
...
interface ConnectOptions {
  ...
  appSalt: string;                        // device-key slot; HKDF salt on Solana/Stellar native
  ...
  vault?: boolean | { url?: string };     // keys in the Cavos vault (needs appId); see /docs/vault
  ...
  // --- Solana-only ---
  relayer?: SolanaRelayer;                // gasless fee payer
  feePayer?: Keypair;                     // self-funded fallback
  passkeyPrf?: PasskeyPrfProvider;        // restores from a passkey's DEK copy (single-chain)

  // --- Stellar-only (classic `G…`) ---
  stellarRelayer?: StellarRelayer;        // gasless sponsorship
  stellarSourceKeypair?: StellarKeypair;  // self-funded source/fee-payer
  stellarDeviceKey?: DeviceUnwrapKey;     // unused by the current signer model; keep only if a low-level caller still passes one
...
Recover after losing every device using a saved recovery code. **Starknet only.**
Native Solana restores by connecting (enclave or passkey). On grandfathered
Stellar, reconnect and call `wallet.approveThisDeviceWithRecovery(code)`.
...
Optional, opt-in per environment. On Solana/Stellar the enclave seals the
MasterDEK. On Starknet it still schedules a restricted `add_signer`. See
...
On Starknet, a second factor for approving new devices. On Solana/Stellar
native, the passkey's PRF encrypts a copy of the MasterDEK that Cavos stores
and cannot open (can spend). Signing up never prompts. See
...
| `approveDeviceEverywhere(wallets, passkey)` | Starknet on-chain assertion batch. Not native Solana. |
| `PasskeyApprovable` | Interface implemented by Starknet `Cavos`. Native `CavosSolana` throws on approver APIs. |
| `PasskeyPrf` | WebAuthn PRF helper. Opens a passkey's DEK copy on native Solana/Stellar. |

## Signers

| Export | Description |
|--------|-------------|
| `WebCryptoSigner` | Browser silent device signer (non-extractable P-256 in IndexedDB). Low-level; with the vault, keys live on the Cavos origin instead. |
...
| Export | Description |
|--------|-------------|
| `VaultClient` | The page's side of the [Cavos vault](/docs/vault). `VaultClient.attach({ appId, url? })` mounts the iframe; used by `Cavos.connect` when `vault` is set. |
| `startVaultHost`, `startVaultConfirm` | From `@cavos/kit/vault`: the code behind the vault's two pages. Cavos serves them; you do not need them to integrate. |

Signers pass whole payloads so the vault can read them: `Ed25519SpendSigner`
has `signTransaction(message)` and `signMessage(message)`, and a Stellar
`ControlKey` has `signTransaction(xdr, networkPassphrase)`,
`signAuthEntry(preimage)` and `signMessage(message)`. `signMessage` applies the
Cavos message prefix itself.
...
| Export | Description |
|--------|-------------|
| `CavosSolana` | Native Ed25519 system account. `connect`, `execute(amount, dest)`, `executeInstructions(...)`, `signMessage` (`ed25519`). `addSigner` / `recover` throw. |
| `ConnectSolanaOptions`, `RecoverSolanaOptions` | Option types (`RecoverSolanaOptions` is deprecated). |
| `InstructionData`, `InstructionAccount` | Serializable instruction shape. |
| `SolanaRelayer`, `SolanaRelayerOptions` | Fee-payer relayer. |
| `SOLANA_NETWORKS`, `SolanaNetwork` | Network configs. |

### Stellar

Classic `G…` account. Native MasterDEK path shares the DEK with Solana;
grandfathered wallets still use extra Horizon signers. See [Stellar](/docs/stellar).

| Export | Description |
|--------|-------------|
| `CavosStellar` | High-level Stellar client. `nativeDek` is true on the current path. Native: `enrollPasskey` / `setupRecovery` no-op; `approveThisDeviceWithPasskey` / `approveThisDeviceWithRecovery` throw — reconnect with passkey or enclave. Grandfathered methods remain for older wallets. |
...
| `PasskeyPrf`, `PasskeyPrfOptions`, `PasskeyPrfEnrollParams` | WebAuthn PRF helper (passkey DEK copy / grandfathered extra signer). |
...
| `NativeDeviceSigner` | Secure Enclave/Keystore P-256 signer (**Starknet**). |
| `NativeControlSigner` | Native ed25519 control key. Used on grandfathered Stellar; native MasterDEK imports Ed25519 into the native signer. |
| `NativePasskeySigner`, `NativePasskeyPrf` | Native passkey adapters. |
| `getNativeCapabilities()` | Reports signing, passkey, and PRF support. |
