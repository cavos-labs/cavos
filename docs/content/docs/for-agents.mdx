...
- **Social recovery is opt-in and separate:** a developer may additionally enable **hardware-isolated social recovery** for exactly one provider per environment (Google, Apple, or email). It runs inside Google Confidential Space, requires four pinned attestation values in `socialRecoveryAttestation`, and `<CavosProvider>` drives enrollment and recovery automatically. Describe it as *hardware-isolated, non-custodial* — never as *trustless*. Don't assume it is on: it is off unless the environment enables it. See [Recovery](/docs/recovery).
