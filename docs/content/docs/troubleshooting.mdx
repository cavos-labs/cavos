...
4. **The active wallet is classic Stellar.** Hardware-isolated recovery is
   Starknet and Solana only. A new `G…` device uses a passkey or a recovery
   code. See [Stellar](/docs/stellar#recovery).
